export const createNewThread = async (selectedRole) => {
  try {
...
//       `http://localhost:8000/api/threads/${threadId}/runs/${runId}`
//     );
//     return response.json();
//   } catch (err) {
//     console.log(err.message);
...
export const postMessage = async (threadId, message) => {
  try {
    let response = await fetch(
      `http://localhost:8000/api/threads/${threadId}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
export const postToolResponse = async (threadId, runId, toolResponses) => {
  try {
    let response = await fetch(
      `http://localhost:8000/api/threads/${threadId}/runs/${runId}/tool`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
